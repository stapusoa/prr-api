export const getFilteredListings = (properties, filters, searchQuery, sortBy) => {
    let results = properties.filter((listing) => {
        const matchesSearch = !searchQuery ||
            listing.title?.toLowerCase().includes(searchQuery.toLowerCase()) ||
            listing.address?.toLowerCase().includes(searchQuery.toLowerCase()) ||
            listing.description?.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesPrice = typeof listing.price === "number" &&
        ...
        const matchesBedrooms = filters.bedrooms === "any" || (listing.bedrooms ?? 0) >= Number(filters.bedrooms);
        const matchesBathrooms = filters.bathrooms === "any" || (listing.bathrooms ?? 0) >= Number(filters.bathrooms);
